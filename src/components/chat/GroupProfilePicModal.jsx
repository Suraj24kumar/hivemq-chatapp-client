...
export default function GroupProfilePicModal({ open, onClose, group, onUpdated }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      const { data } = await api.patch(`/groups/${group._id}/profile-pic`, form);
      onUpdated?.(data);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50" onClick={onClose}>
      ...
      >
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Group picture</h3>
        <div className="flex justify-center mb-4">
          <Avatar src={group.profilePic} name={group.name} className="w-24 h-24 rounded-full" />
        </div>
